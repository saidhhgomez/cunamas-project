...
import ModalMensaje from '../app/components/ModalMensaje'; 
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const subscripcionExpiracion = DeviceEventEmitter.addListener('EXPIRAR_SESION_GLOBAL', (data) => {
      setModalSesion({
        visible: true,
        titulo: data?.titulo || 'Sesión Expirada',
        mensaje: data?.mensaje || 'Por favor, vuelve a iniciar sesión.',
        tipo: 'logout',
      });
    });

    const subscripcionRefreshReintento = DeviceEventEmitter.addListener('REFRESH_REINTENTAR', (data) => {
      setModalSesion({
        visible: true,
        titulo: data?.titulo || 'Intenta nuevamente',
        mensaje: data?.mensaje || 'No se pudo actualizar tu sesión por un problema de red. Intenta nuevamente sin cerrar la sesión.',
        tipo: 'retry',
      });
    });
    ...
    return () => {
      ...
      subscripcionRefreshReintento.remove();
  ...
  return (
    <AuthContext.Provider value={{ user, login, logout, isLoading, updateUser }}>
      ...
      <ModalMensaje
        ...
        tipo="atencion"
        titulo={modalSesion.titulo}
        mensaje={modalSesion.mensaje}
        onCerrar={async () => {
          setModalSesion((prev) => ({ ...prev, visible: false }));

          if (modalSesion.tipo === 'retry') {
            return;
          }
