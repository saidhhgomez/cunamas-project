...
export const DistritoService = {
  ...
  registrarDireccion: async (datos: DireccionRequest): Promise<DireccionCreadaResponse> => {
    try {
      console.log('[Direcciones] POST /direcciones - payload:', datos);
      const response = await api.post<DireccionCreadaResponse>('/direcciones', datos);
      console.log('[Direcciones] Respuesta exitosa:', response.status, response.data);
      return response.data;
    } catch (error: any) {
      console.error('[Direcciones] Detalle HTTP:', {
