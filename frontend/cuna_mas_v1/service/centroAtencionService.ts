...
export const LocalService = {
  ...
  registrarLocalBasico: async (datos: LocalBasicoRequest): Promise<LocalResponse> => {
    try {
      console.log('[Locales] POST /centros-atencion-infantil - payload:', datos);
      const response = await api.post<LocalResponse>('/centros-atencion-infantil', datos);
      console.log('[Locales] Respuesta exitosa:', response.status, response.data);
      return response.data;
    } catch (error: any) {
      console.error('[Locales] Detalle HTTP:', {
