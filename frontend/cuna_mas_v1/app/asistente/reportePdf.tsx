...
import {
  View, Text, TouchableOpacity, StyleSheet, ActivityIndicator, StatusBar, Platform, ScrollView, Modal
} from 'react-native';
...
const CORRELATIVOS = [
  { value: 1, label: 'Media Mañana', icon: 'sunny-outline' },
];
...
export default function SeleccionarReportePdf() {
  ...
  const { user } = useAuth();
  const { mostrarError, mostrarAtencion, mostrarExito, modalProps } = useModalMensaje();
  ...
  const avisarCambioYResetear = (accionQueCambia: () => void) => {
    if (reporteGenerado) {
      mostrarAtencion('Atención', 'Cambiaste un filtro. Debes volver a generar el reporte.');
    }
    accionQueCambia();
  ...
  const manejarGenerar = async () => {
    try {
      ...
      mostrarError('Error', err?.message || 'No se pudo generar el reporte. Intenta nuevamente.');
    } finally {
      setGenerando(false);
    }
  };
    ...
    try {
      ...
      if (Platform.OS === 'android') {
        ...
        try {
          ...
          const permisos = await FileSystem.StorageAccessFramework.requestDirectoryPermissionsAsync();
          if (!permisos.granted) {
            mostrarAtencion('Permiso necesario', 'Debes seleccionar una carpeta para guardar el archivo.');
            return;
          }
        ...
        if (!disponible) {
          mostrarAtencion('No disponible', 'Compartir archivos no está disponible en este dispositivo.');
          return;
        }
      ...
      mostrarError('Error', 'No se pudo guardar el archivo.');
    } finally {
      setGuardando(false);
    }
  };
  ...
  const centroSeleccionado = centros.find(
    (centro) => obtenerIdServicioAlimentario(centro) === idCentro
  );
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.titleBar}>
        <Text style={styles.headerTitle} allowFontScaling={false}>Generar Reporte</Text>
      ...
      >
        ...
        <View style={[styles.pickerWrapper, reporteGenerado && styles.deshabilitado]}>
          {cargandoCentros ? (
            ...
            <TouchableOpacity
              style={styles.servicioSelector}
              onPress={() => setModalServiciosVisible(true)}
              disabled={reporteGenerado}
              activeOpacity={0.8}
            >
              <Text
                style={[styles.servicioTexto, !centroSeleccionado && styles.servicioPlaceholder]}
                numberOfLines={1}
                adjustsFontSizeToFit
                minimumFontScale={0.7}
                maxFontSizeMultiplier={1.15}
              >
                {centroSeleccionado?.nombreCentro || 'Seleccione un servicio alimentario'}
              </Text>
              <Ionicons name="chevron-down" size={20} color="#006080" />
            </TouchableOpacity>
          )}
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalServicios}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitulo}>Seleccione un servicio alimentario</Text>
              <TouchableOpacity
                onPress={() => setModalServiciosVisible(false)}
                style={styles.modalCerrar}
                accessibilityLabel="Cerrar selector de servicios"
              >
                <Ionicons name="close" size={24} color="#006080" />
              </TouchableOpacity>
            </View>
            <ScrollView showsVerticalScrollIndicator={false}>
              {centros.map((centro) => {
                const seleccionado = obtenerIdServicioAlimentario(centro) === idCentro;
                return (
                  <TouchableOpacity
                    key={obtenerIdServicioAlimentario(centro)}
                    style={[styles.servicioOpcion, seleccionado && styles.servicioOpcionActiva]}
                    onPress={() => {
                      avisarCambioYResetear(() => setIdCentro(obtenerIdServicioAlimentario(centro)));
                      setModalServiciosVisible(false);
                    }}
                  >
                    <Text style={styles.servicioOpcionTexto}>{centro.nombreCentro}</Text>
                    {seleccionado && <Ionicons name="checkmark" size={20} color="#006080" />}
                  </TouchableOpacity>
                );
              })}
            </ScrollView>
...
const styles = StyleSheet.create({
  ...
  servicioSelector: { minHeight: 50, paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center' },
  servicioTexto: { flex: 1, minWidth: 0, marginRight: 8, color: '#1E293B', fontSize: 14, fontWeight: '600' },
  servicioPlaceholder: { color: '#64748B' },
  ...
  modalServicios: { backgroundColor: '#FFFFFF', borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: '75%', paddingBottom: 24 },
  ...
  modalTitulo: { flex: 1, marginRight: 12, color: '#006080', fontSize: 18, fontWeight: '800' },
  modalCerrar: { padding: 4 },
  servicioOpcion: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: '#F1F5F9' },
  servicioOpcionActiva: { backgroundColor: '#EEF7FA' },
  servicioOpcionTexto: { flex: 1, flexShrink: 1, marginRight: 12, color: '#1E293B', fontSize: 15, lineHeight: 21, fontWeight: '600' },
});
