...
export default function UsuariosPendientes() { 
  ...
  const cargarUsuarios = async () => {
    try {
      ...
      mostrarError('Error', 'No se pudo obtener la lista de usuarios pendientes.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const confirmarDenegar = (item) => {
    mostrarConfirmacion(
      ...
      () => denegarUsuario(item),
      { textoConfirmar: 'DENEGAR', variantePeligro: true }
    );
  };
  ...
  const denegarUsuario = async (item) => {
    try {
      ...
      mostrarExito('Usuario Denegado', `${item.nombresCompletos} fue denegado correctamente.`);
    } catch (error) {
      mostrarError('Error', 'No se pudo denegar al usuario. Intenta nuevamente.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const renderUserItem = ({ item }) => ( 
    ...
    > 
      ...
      <View style={styles.userInfo}> 
        <Text style={styles.userName}>{item.nombresCompletos}</Text> 
...
<View style={styles.titleBar}>
  <Text style={styles.headerTitle} allowFontScaling={false}>Usuarios Pendientes</Text> 
</View>
