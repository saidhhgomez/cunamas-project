...
export default function ActivarCuentaMartina() {
  ...
  useEffect(() => {
    const cargarDetalleUsuario = async () => {
      if (!idPersona) {
        mostrarError('Error', 'No se encontró el identificador del usuario.');
        router.back();
        return;
      }
      ...
      try {
        ...
        mostrarError('Error', 'No se pudo recuperar la información detallada del usuario.');
        router.back();
      } finally {
        setLoading(false);
      }
  ...
  const handleActivarCuenta = async () => {
    ...
    if (selectedRoleId === null) {
      mostrarAtencion('Atención', 'Por favor, selecciona un rol para el usuario.');
      return;
    }
    ...
    try {
      ...
      mostrarExito('Éxito', 'Usuario aprobado y cuenta activada correctamente.', {
        textoBoton: 'CONTINUAR',
        onCerrar: () => router.back(),
      });
    } catch (error) {
      mostrarError('Error', 'Ocurrió un problema al intentar activar la cuenta.');
    } finally {
  ...
  return ( 
    <View style={[styles.container, { paddingTop: insets.top }]}> 
      ...
      <View style={styles.titleBar}>
        <Text style={styles.headerTitle} allowFontScaling={false}>Activar Cuenta</Text>
      </View>
