...
export default function CalculadoraUnificada() { 
  ...
  const { user } = useAuth(); 
  const { mostrarError, mostrarExito, modalProps } = useModalMensaje();
  ...
  const manejarCalcular = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        mostrarExito('Éxito', 'Cálculo procesado y añadido a la lista JSON.');
  ...
  const manejarEnviarAIA = async () => {
    try {
      ...
      mostrarError('Error', 'No se pudo procesar el análisis con la IA.');
    } finally {
      setLoadingIA(false);
    }
  };
